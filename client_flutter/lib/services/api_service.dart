...
class ApiService {
  ...
  static Future<LastHomeSession?> loadLastHomeSession() async {
    ...
    if (code == null || code.trim().isEmpty || name == null || member == null)
      return null;
    return LastHomeSession(
      serverUrl: prefs.getString('last_server_url') ??
  ...
  Future<AppCapabilities> getCapabilities() async {
    final response = await http
        .get(_uri('/app/capabilities'), headers: _headers)
  ...
  Future<ServerSyncStatus> getServerSyncStatus() async {
    try {
      final response = await http
          .get(_uri('/app/sync-status'), headers: _headers)
          .timeout(const Duration(seconds: 8));
      if (response.statusCode != 200) throw Exception(_extractError(response));
  ...
  Future<Member> createMember(
    ...
    final response = await http.post(
      ...
      headers: _headers,
      body:
          jsonEncode({'name': name, 'pin': pin, 'color': color, 'role': role}),
    );
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return Member.fromJson(jsonDecode(response.body));
  }
  ...
  Future<Member> updateMember(
    ...
    final response = await http.patch(_uri('/household/members/$memberId'),
        headers: _headers, body: jsonEncode(body));
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return Member.fromJson(jsonDecode(response.body));
  }

  Future<Member> setMemberActive(
      {required int memberId,
  ...
  Future<void> saveMyIncome(
      {required String month, required double amount, String? note}) async {
  ...
  Future<List<IncomeItem>> getIncome(String month) async {
    final response = await http.get(_uri('/finance/income', {'month': month}),
        headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return (jsonDecode(response.body) as List)
  ...
  Future<List<FixedExpenseTemplateItem>> getFixedExpenses(
      {bool activeOnly = true}) async {
    final response = await http.get(
        _uri('/finance/fixed-expenses', {'active_only': activeOnly.toString()}),
        headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return (jsonDecode(response.body) as List)
        .map((item) => FixedExpenseTemplateItem.fromJson(item))
        .toList();
  }
  ...
  Future<FixedExpenseTemplateItem> updateFixedExpense({
    ...
    final response = await http.patch(
        _uri('/finance/fixed-expenses/$templateId'),
        headers: _headers,
        body: jsonEncode(body));
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return FixedExpenseTemplateItem.fromJson(jsonDecode(response.body));
  ...
  Future<ExpenseItem> generateFixedExpense(
      {required int templateId, required String month}) async {
    final response = await http.post(
        _uri('/finance/fixed-expenses/$templateId/generate', {'month': month}),
        headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
  ...
  Future<List<ExpenseItem>> generateFixedExpensesForMonth(String month) async {
    final response = await http.post(
        _uri('/finance/fixed-expenses/generate-for-month', {'month': month}),
        headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return (jsonDecode(response.body) as List)
  ...
  Future<CardImportPreviewResult> previewCardImportPdf(
      {required Uint8List bytes,
      required String filename,
      String? month}) async {
    final request = http.MultipartRequest(
        'POST',
        _uri('/finance/card-imports/preview',
  ...
  Future<List<ExpenseItem>> getExpenses(String month) async {
    final response = await http.get(_uri('/finance/expenses', {'month': month}),
        headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return (jsonDecode(response.body) as List)
        .map((item) => ExpenseItem.fromJson(item))
  ...
  Future<void> deleteExpense(int expenseId) async {
    final response = await http.delete(_uri('/finance/expenses/$expenseId'),
        headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
  }

  Future<MonthSummary> getSummary(String month) async {
    final response = await http.get(_uri('/finance/summary', {'month': month}),
        headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return MonthSummary.fromJson(jsonDecode(response.body));
  }

  Future<List<MemberParticipationItem>> getParticipation(String month,
      {bool includeInactive = false}) async {
    final response = await http.get(
      _uri('/finance/participation',
          {'month': month, 'include_inactive': includeInactive.toString()}),
      headers: _headers,
    );
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return (jsonDecode(response.body) as List)
        .map((item) => MemberParticipationItem.fromJson(item))
        .toList();
  }
  ...
  Future<void> setParticipation(
    ...
    final response = await http.put(
      ...
      headers: _headers,
      body: jsonEncode(
          {'month': month, 'participates': participates, 'note': note}),
    );
    if (response.statusCode != 200) throw Exception(_extractError(response));
  }
  ...
  Future<List<DebtItem>> getDebts({bool includeCancelled = false}) async {
    final response = await http.get(
      _uri(
          '/finance/debts', {'include_cancelled': includeCancelled.toString()}),
      headers: _headers,
    );
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return (jsonDecode(response.body) as List)
  ...
  Future<void> addDebtPayment({
    ...
    final response = await http.post(
      ...
      headers: _headers,
      body:
          jsonEncode({'amount': amount, 'date': _dateOnly(date), 'note': note}),
    );
    if (response.statusCode != 200) throw Exception(_extractError(response));
  }

  Future<List<DebtPaymentItem>> getDebtPayments(int debtId) async {
    final response = await http.get(_uri('/finance/debts/$debtId/payments'),
        headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return (jsonDecode(response.body) as List)
        .map((item) => DebtPaymentItem.fromJson(item))
        .toList();
  }

  Future<void> confirmDebtPayment(
      {required int debtId, required int paymentId}) async {
    final response = await http.post(
        _uri('/finance/debts/$debtId/payments/$paymentId/confirm'),
        headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
  }

  Future<void> rejectDebtPayment(
      {required int debtId, required int paymentId, String reason = ''}) async {
    final response = await http.post(
  ...
  Future<List<CreditBalanceItem>> getCreditBalances(
      {bool activeOnly = true}) async {
    final response = await http.get(
        _uri(
            '/finance/credit-balances', {'active_only': activeOnly.toString()}),
        headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return (jsonDecode(response.body) as List)
        .map((item) => CreditBalanceItem.fromJson(item))
        .toList();
  }

  Future<Map<String, dynamic>> reconcileAdvanceCredits({String? month}) async {
    final response = await http.post(
      _uri('/finance/credit-balances/reconcile',
          {if (month != null) 'month': month}),
      headers: _headers,
    );
    if (response.statusCode != 200) throw Exception(_extractError(response));
  ...
  Future<void> applyCreditBalance(
  ...
  Future<MonthlyCloseItem> closeMonth(String month,
      {bool advanceToNext = true}) async {
    final response = await http.post(
  ...
  Future<List<MonthlyCloseItem>> getMonthlyCloses() async {
    final response =
        await http.get(_uri('/finance/monthly-closes'), headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return (jsonDecode(response.body) as List)
        .map((item) => MonthlyCloseItem.fromJson(item))
        .toList();
  }

  Future<List<MonthPeriodStatusItem>> getMonthPeriodStatuses() async {
    final response =
        await http.get(_uri('/finance/month-periods'), headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return (jsonDecode(response.body) as List)
        .map((item) => MonthPeriodStatusItem.fromJson(item))
        .toList();
  }
  ...
  Future<HouseholdPeriodSettingsItem> getPeriodSettings() async {
    final response =
        await http.get(_uri('/finance/period-settings'), headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return HouseholdPeriodSettingsItem.fromJson(jsonDecode(response.body));
  }

  Future<HouseholdPeriodSettingsItem> updatePeriodSettings(
      {required String periodMode, required int startDay}) async {
    final response = await http.put(
  ...
  Future<HouseholdPeriodSettingsItem> getActivePeriod() async {
    final response =
        await http.get(_uri('/finance/active-period'), headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return HouseholdPeriodSettingsItem.fromJson(jsonDecode(response.body));
  }

  Future<Map<String, dynamic>> resetActivePeriodBasic() async {
    final response = await http.post(_uri('/finance/active-period/reset-basic'),
        headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return jsonDecode(response.body) as Map<String, dynamic>;
  }

  Future<List<MonthlyAdvancePaymentItem>> getMonthlyAdvancePayments(
      String month) async {
    final response = await http.get(
        _uri('/finance/monthly-advance-payments', {'month': month}),
        headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return (jsonDecode(response.body) as List)
        .map((item) => MonthlyAdvancePaymentItem.fromJson(item))
        .toList();
  }

  Future<void> createMonthlyAdvancePayment(
      {required String month,
      required int receivedByMemberId,
      required double amount,
      required DateTime date,
      String note = ''}) async {
    final response = await http.post(
  ...
  Future<void> confirmMonthlyAdvancePayment(int paymentId) async {
    final response = await http.post(
        _uri('/finance/monthly-advance-payments/$paymentId/confirm'),
        headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
  }

  Future<void> rejectMonthlyAdvancePayment(
      {required int paymentId, String reason = ''}) async {
    final response = await http.post(
  ...
  Future<List<HouseholdTaskItem>> getTasks(
      {bool includeDone = false, bool assignedToMe = false}) async {
    final response = await http.get(
      _uri('/tasks', {
        'include_done': includeDone.toString(),
    ...
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return (jsonDecode(response.body) as List)
        .map((item) => HouseholdTaskItem.fromJson(item))
        .toList();
  }
  ...
  Future<HouseholdTaskItem> refreshTaskAi(int taskId) async {
    final response =
        await http.post(_uri('/tasks/$taskId/ai-refresh'), headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return HouseholdTaskItem.fromJson(jsonDecode(response.body));
  }
  ...
  Future<void> completeTask(int taskId) async {
    final response =
        await http.post(_uri('/tasks/$taskId/complete'), headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
  }

  Future<void> cancelTask(int taskId) async {
    final response =
        await http.post(_uri('/tasks/$taskId/cancel'), headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
  }

  Future<List<AiReportItem>> getHouseholdAiReports(String month) async {
    final response = await http.get(
        _uri('/ai/household-reports', {'month': month}),
        headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return (jsonDecode(response.body) as List)
        .map((item) => AiReportItem.fromJson(item))
        .toList();
  }

  Future<AiReportItem> createHouseholdAiReport(
      {required String month,
  ...
  Future<AiWeeklySettings> getWeeklyAiSettings() async {
    final response =
        await http.get(_uri('/ai/weekly-settings'), headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
  ...
  Future<AiWeeklySettings> updateWeeklyAiSettings({
    ...
    if (analysisFrequency != null)
      body['analysis_frequency'] = analysisFrequency;
    if (preferredWeekday != null) body['preferred_weekday'] = preferredWeekday;
    if (useExternalContext != null)
      body['use_external_context'] = useExternalContext;
    if (useNewsContext != null) body['use_news_context'] = useNewsContext;
    final response = await http.put(_uri('/ai/weekly-settings'),
        headers: _headers, body: jsonEncode(body));
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return AiWeeklySettings.fromJson(jsonDecode(response.body));
  }
  ...
  Future<AiWeeklyReportResult> getLatestWeeklyAiReport() async {
    final response =
        await http.get(_uri('/ai/weekly-latest'), headers: _headers);
    if (response.statusCode != 200) throw Exception(_extractError(response));
    return AiWeeklyReportResult.fromJson(jsonDecode(response.body));
  }

  Future<AiWeeklyReportResult> createWeeklyAiReport(
      {required String month, bool force = false, bool useApi = true}) async {
    final response = await http
        .post(
  ...
  Future<AiWeeklyReportResult> refreshWeeklyAiIfNeeded(
      {required String month, bool useApi = true}) async {
    final response = await http
        .post(
  ...
  Future<Map<String, dynamic>> exportHouseholdBackup() async {
    try {
      final response = await http
          .get(_uri('/backup/household'), headers: _headers)
